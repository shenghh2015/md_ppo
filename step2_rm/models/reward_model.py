...
class ValuleHead(torch.nn.Module):
  def __init__(self, args) -> None:
    super().__init__()
    self.args = args
    self.dense = ParallelLinear(args.hidden_size,
                                1,
                                bias=True,
                                input_is_parallel=False,
                                args=args)
...
class GPTModelCritic(GPTModel):
  ...
  def __init__(self,
    ...
    if self.post_process:
      self.v_head = ValuleHead(args)

  def reward_loss(self, input_ids, rewards, pad_id, score_labels, sample_freqs):
    """
    计算reward模型的loss
    Args:
    ...
    chosen_scores = score_labels[:bs]
    reject_scores = score_labels[bs:]
    chosen_freqs  = sample_freqs[:bs]
    reject_freqs  = sample_freqs[bs:]
    
    # print(chosen_scores, reject_scores)
    for i in range(bs):
      # print(i)
      ...
      # assert divergence_ind >= 0  # fix the bug when the prompt = ''
      # c_truncated_reward = chosen_reward[divergence_ind:end_ind]
      # r_truncated_reward = rejected_reward[divergence_ind:end_ind]

      c_last_token_reward = chosen_reward[c_ind - 1]
      r_last_token_reward = rejected_reward[r_ind - 1]
      ...
      reject_score = reject_scores[i]
      # reward loss for chosen_score > reject_score
      r_loss1 = - torch.nn.functional.logsigmoid(c_last_token_reward - r_last_token_reward)
      # reward loss for chosen_score == reject_score
      r_loss2 = torch.abs(c_last_token_reward - r_last_token_reward).mean()
      # paired reward loss
      r_loss = r_loss1 * (chosen_score > reject_score) + r_loss2 * (chosen_score == reject_score) * self.args.equal_score_loss_weight
      
      # 0-positive penalty
      if self.args.zero_positive_penalty_weight > 0.:
        chosen_freq  = chosen_freqs[i]
        reject_freq  = reject_freqs[i]
        label_scores = [chosen_score, reject_score]
        freqs  = [chosen_freq, reject_freq]
        reward_scores = [c_last_token_reward, r_last_token_reward]   # mean of token-wise reward scores
        for rm_score, lb_score, freq in zip(reward_scores, label_scores, freqs):
          penalty = torch.exp(rm_score) * (lb_score == 0) + torch.exp(-rm_score) * (lb_score > 0)
          r_loss += self.args.zero_positive_penalty_weight * penalty / freq
      
      # bound penalty
      if self.args.reward_bound_penalty_weight > 0.:
        chosen_freq  = chosen_freqs[i]
        reject_freq  = reject_freqs[i]
        label_scores = [chosen_score, reject_score]
        freqs  = [chosen_freq, reject_freq]
        reward_scores = [c_last_token_reward, r_last_token_reward] # mean of token-wise reward scores
        for rm_score, lb_score, freq in zip(reward_scores, label_scores, freqs):
          penalty = (torch.abs(rm_score) > self.args.reward_bound) * torch.exp(torch.abs(rm_score) - self.args.reward_bound)
          r_loss += self.args.reward_bound_penalty_weight * penalty / freq
        
      # incompletion penalty
      if self.args.reward_self_supervised_weight > 0.:
        check_divergence = (chosen_id != rejected_id).nonzero()
        if len(check_divergence) > 0:
          c_divergence_ind = check_divergence[0]
          if c_divergence_ind > 0 and c_ind - c_divergence_ind > 2:
            chosen_answer_reward = chosen_reward[c_divergence_ind: c_ind]
            score_difs = chosen_answer_reward[1: c_ind].reshape((-1, 1)) - chosen_answer_reward[: c_ind - 1].reshape((1, -1)) < 0
            mask = torch.tril(torch.ones_like(score_difs), diagonal=-1) > 0
            neighbor_loss = torch.sum(score_difs * mask) / torch.sum(mask)
            r_loss += self.args.reward_self_supervised_weight * neighbor_loss
  ...
  def forward(
    ...
    if self.post_process:
      ...
      if reward_model_training:
        loss = self.reward_loss(input_ids=input_ids,
                                rewards=v_head_output,
                                pad_id=pad_id,
  ...
  def forward_eval(self,
    ...
    if self.post_process:
      ...
      v_head_output = v_head_output.squeeze(-1)
      # eval_loss
      eval_loss = self.reward_loss(input_ids=input_ids,
                                   rewards=v_head_output,
                                   pad_id=pad_id,
                                   score_labels=score_labels,
                                   sample_freqs=sample_freqs)
      ...
      valid_chosen_end_scores = []
      valid_reject_end_scores = []
      chosen_scores = score_labels[:bs]
      reject_scores = score_labels[bs:]
      ...
      for i in range(bs):
        ...
        if chosen_scores[i] > reject_scores[i]:
          valid_chosen_end_scores.append(chosen_end_scores[-1])
          valid_reject_end_scores.append(reject_end_scores[-1])
      ...
      return {
          "chosen_end_scores": valid_chosen_end_scores,
          "reject_end_scores": valid_reject_end_scores,
          "loss": eval_loss.item()
      }
  ...
  def forward_value(
    ...
    if self.post_process:
      ...
      for i in range(bs):
        ...
        v_head_outputs.append(value[prompt_length:c_ind].tolist())
      
      return {
          "values": v_head_outputs,
  ...
  def state_dict_for_save_checkpoint(self,
    ...
    if hasattr(self, 'v_head'):
      state_dict_['v_head'] = self.v_head.state_dict(destination, prefix,
                                                     keep_vars)
      # if self.args.use_v_head_layernorm:
