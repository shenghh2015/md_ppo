...
def _multi_tensor_copy_this_to_that(this, that, overflow_buf=None):
  ...
  if overflow_buf:
    ...
    multi_tensor_applier(amp_C.multi_tensor_scale, overflow_buf, [this, that],
                         1.0)
  else:
...
class MegatronOptimizer(ABC):
  ...
  def __init__(self, optimizer, clip_grad, log_num_zeros_in_grad,
    ...
    # print('params_have_main_grad',params_have_main_grad)
    # print(sg)
...
class Float16OptimizerWithFloat16Params(MegatronOptimizer):
  ...
  def __init__(self, optimizer, clip_grad, log_num_zeros_in_grad,
    ...
    super(Float16OptimizerWithFloat16Params,
          self).__init__(optimizer, clip_grad, log_num_zeros_in_grad,
                         params_have_main_grad)
      ...
      for i, param in enumerate(param_group['params']):
        if param.requires_grad:
          ...
          if param.type() in [
            ...
            # Create a copy and convert to fp32
            ...
            # print('diff main and model sdfgsfs',(main_param.data.half()-param.data).abs().mean(),(main_param.data-param.data.float()).abs().mean())
    ...
    # self.print_diff_btw_model_and_main()
  ...
  def _copy_model_grads_to_main_grads(self):
    ...
    # fp32的group默认不需要进行转换操作
    for model_group, main_group in zip(self.float16_groups,
                                       self.fp32_from_float16_groups):
      for model_param, main_param in zip(model_group, main_group):
        if self.params_have_main_grad:
          # print('model_param.name',model_param.name,model_param.main_grad.sum(),model_param.shape)
          # 将模型的梯度拷贝到备份参数的
  ...
  def print_diff_btw_model_and_main(self):
    model_data, main_data = self._get_model_and_main_params_data_float16()
    for model_data_,main_data_ in zip(model_data,main_data):
      print('diff main/model',(model_data_.float()-main_data_).abs().mean().item(),(model_data_-main_data_.half()).abs().mean().item())
  ...
  def step(self):
    ...
    #       'lr',lr,
    #       'param grad mean', 
    #       param.grad.abs().mean() if param.grad is not None else None,
    #       )
    # print(sfg)
    # Step the optimizer.

    # import inspect
    # print(f'use optimizer class: {self.optimizer}',self.optimizer.__module__)
    # print('step path',inspect.getabsfile(self.optimizer.step))
    # print(srg)

    # p0_main = self.fp32_from_float16_groups[0][0].data.clone()
    # p0_model = self.float16_groups[0][0].data.clone()
    # print_rank_0(f'diff main and model before update {(p0_main-p0_model.float()).abs().mean()}')
    # p_test_data = p_test.clone()
    # print('p0 grad',p_test.grad.abs().mean())
    self.optimizer.step()
    # p0_main_after = self.fp32_from_float16_groups[0][0].data.clone()
    # p0_model_after = self.float16_groups[0][0].data.clone()
    # print_rank_0(f'diff main after step {(p0_main_after-p0_main).abs().mean(),(p0_model_after-p0_model).abs().mean()}')
