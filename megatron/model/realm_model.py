...
def general_ict_model_provider(only_query_model=False, only_block_model=False):
  ...
  model = ICTBertModel(ict_head_size=args.ict_head_size,
                       num_tokentypes=2,
...
class ICTBertModel(MegatronModule):
  ...
  def __init__(self,
    ...
    bert_kwargs = dict(ict_head_size=ict_head_size,
  ...
  def forward(self, query_tokens, query_attention_mask, block_tokens,
  ...
  def embed_query(self, query_tokens, query_attention_mask):
    ...
    if self.use_query_model:
      ...
      query_ict_logits, _ = self.query_model.forward(query_tokens,
  ...
  def embed_block(self, block_tokens, block_attention_mask):
    ...
    if self.use_block_model:
      ...
      block_ict_logits, _ = self.block_model.forward(block_tokens,
                                                     block_attention_mask,
                                                     block_types)
  ...
  def load_state_dict(self, state_dict, strict=True):
    ...
    if self.use_query_model:
      ...
      self.query_model.load_state_dict(state_dict[self._query_key],
                                       strict=strict)

    if self.use_block_model:
      print("Loading ICT block model", flush=True)
      self.block_model.load_state_dict(state_dict[self._block_key],
                                       strict=strict)
...
class IREncoderBertModel(MegatronModule):
  ...
  def __init__(self, ict_head_size, num_tokentypes=2, parallel_output=True):
    ...
    self.ict_head = get_linear_layer(args.hidden_size, ict_head_size,
                                     init_method)
  ...
  def forward(self, input_ids, attention_mask, tokentype_ids=None):
    ...
    lm_output, pooled_output = self.language_model(input_ids,
                                                   position_ids,
