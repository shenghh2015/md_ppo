...
def get_language_model(num_tokentypes,
  ...
  # args = get_args()
  assert init_method is not None and scaled_init_method is not None
...
class Embedding(MegatronModule):
  ...
  def __init__(self,
    ...
    self.word_embeddings = mpu.VocabParallelEmbedding(
        vocab_size, self.hidden_size, init_method=self.init_method,args=args)
    self._word_embeddings_key = 'word_embeddings'

    # Position embedding (serial).
    self.position_embedding_type = self.position_embedding_type
    if self.position_embedding_type == PositionEmbeddingType.absolute:
      max_position_embeddings = self.max_position_embeddings
      assert max_position_embeddings is not None
      self.position_embeddings = torch.nn.Embedding(max_position_embeddings,
                                                    self.hidden_size)
  ...
  def add_tokentype_embeddings(self, num_tokentypes):
    ...
    if torch.distributed.get_rank() == 0:
      print('adding embedding for {} tokentypes'.format(num_tokentypes),
            flush=True)
    self.num_tokentypes = num_tokentypes
    self.tokentype_embeddings = self.torch.nn.Embedding(
  ...
  def load_state_dict(self, state_dict, strict=True):
    ...
    # print_with_rank(state_dict_.keys())
...
class EmbeddingWithLMheadPipe(EmbeddingPipe):
  ...
  def __init__(self,
    ...
    self.logits_helper = logits_helper
            
  def forward(self, inputs, **kwargs):
    ret = super().forward(inputs, **kwargs)
    if getattr(self._args, 'pretrain_causal_attention', False):
      embeddings = ret
    else:
      embeddings, _ = ret
...
class TransformerLanguageModel(MegatronModule):
  ...
  def __init__(self,
    ...
    if self.pre_process: 
      self.embedding = Embedding(self.hidden_size, self.padded_vocab_size,
                                 self.hidden_dropout, self.init_method,
                                 self.num_tokentypes,
                                 args=args
                                 )
  ...
  def forward(self,
    ...
    # Embeddings.
    if self.pre_process:
      # print_with_rank('embedding 计算')
      
      embedding_output = self.embedding(enc_input_ids,
                                        enc_position_ids,
                                        tokentype_ids=tokentype_ids)
      # print('embedding_output',embedding_output[:5,:5])
      # print(sfdb)
  ...
  def load_state_dict(self, state_dict, strict=True):
    ...
    # 需要针对lora策略进行操作
    missing_keys, unexpected_keys = self.encoder.load_state_dict(state_dict_, strict=False)
    assert len(unexpected_keys) == 0, f'unexpected_keys:\n{unexpected_keys}'
    args = get_args()
    if getattr(args,'use_lora',False):
      for key in missing_keys:
        assert 'lora_A' in key or 'lora_B' in key, f'only lora param allow to unload, not key: {key}'
