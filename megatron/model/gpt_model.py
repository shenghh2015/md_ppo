...
class GPTModel(MegatronModule):
  ...
  def forward(self,
    ...
    if self.post_process:
      return post_language_model_processing(lm_output, labels,
...
def get_cross_entropy(is_prefix: bool):
  ...
  def CrossEntropy(output, labels):
    ...
    args = get_args()
    # print('get_cross_entropy',labels.shape,loss_mask.shape,output.shape,output[:2,:2,:2])
      ...
      if args.loss_on_targets_only:
        ...
        if args.reweight_loss_based_on_position_frequency:
          reweight = torch.arange(
              ...
              -1,
              dtype=torch.float,
              device=loss_mask.device) / (sequence_length + 1) * 2
...
class GPTModelPipe(PipelineModule, MegatronModule):
  ...
  def __init__(self,
    ...
    if args.fp32_residual_connection:
      if getattr(args, 'pretrain_causal_attention', False):
        ...
        self.specs.append(lambda x:
                          (x[0].transpose(0, 1).contiguous().float(), *x[1:]))
    else:
      if getattr(args, 'pretrain_causal_attention', False):
    ...
    self.specs.append(
        LayerSpec(LayerNorm, args.hidden_size, eps=args.layernorm_epsilon,args=args))

    def _logits_helper(embedding, lm_output):
      """A wrapper to massage inputs/outputs from pipeline. """
      return parallel_lm_logits(lm_output, embedding.word_embeddings_weight,
                                self.parallel_output)
