...
from megatron import (get_args, mpu, print_rank_0, update_num_microbatches,
                      utils, get_tokenizer,print_with_rank,get_server_ip)
...
def save_checkpoint(iteration, model, optimizer, lr_scheduler, tag=None):
  ...
  def _save_checkpoint():
    ...
    if args.deepspeed:
      ...
      # print_with_rank('enter deepspeed save_checkpoint')
      if args.no_save_optim:
        # 不保存deepspeed的zero参数
        model[0].save_zero_checkpoint = False
      model[0].save_checkpoint(checkpoint_name, client_state=state_dict, tag=tag)
  ...
  #     gc.collect()
  #     print_with_rank(f'get lock on :{get_server_ip()}, saving')
  #     _save_checkpoint()
  ...
  if (not args.deepspeed and (not torch.distributed.is_initialized()
...
def load_checkpoint(model,
  ...
  if args.deepspeed:
    # print_rank_0(f'load_dir==========={load_dir}')
    load_optimizer_states = False if args.no_load_optim else True
    loaded_dir, state_dict = model[0].load_checkpoint(
        load_dir, load_optimizer_states=load_optimizer_states,
        load_module_strict = strict # False if getattr(args,'use_lora',False) else True
        )
    ...
    print(
        f'rank: {torch.distributed.get_rank()} load checkpoint: {checkpoint_name}'
    )
    # Load the checkpoint.
    try:
  ...
  if args.finetune or release:
    ...
    try:
      ...
      try:  # Backward compatible with older checkpoints
        ...
        # print_rank_0('Set the iteration to 0')
        # iteration = 0
        sys.exit()
  ...
  # add by zx
  if 'args' in state_dict and not (args.finetune or release) and state_dict['args'] is not None:
    ...
    args.gigaflos_no_embeds = getattr(checkpoint_args, 'gigaflos_no_embeds', 0)
  else:
  ...
  if not args.deepspeed:
    if not release and not args.finetune and not args.no_load_optim:
      ...
      # add by zx, deepspeed-megatron bug !
