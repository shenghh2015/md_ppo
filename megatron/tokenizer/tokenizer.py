...
def build_tokenizer(args):
  ...
  if args.tokenizer_type == 'BertWordPieceLowerCase':
    ...
    from megatron.tokenizer.llama_tokenizer import LLAMATokenizer
    tokenizer = LLAMATokenizer(model_path=args.vocab_file)
  else:
  ...
  # print(tokenizer.vocab_size,args.padded_vocab_size)
  # print(sg)
...
def _vocab_size_with_padding(orig_vocab_size, args):
  ...
  if args.pad_vocab_size_to is not None:
    ...
    if args.make_vocab_size_divisible_by is not None and (
      ...
      raise ValueError(
          f"{args.pad_vocab_size_to} is not divisible by {args.make_vocab_size_divisible_by}"
      )
...
class _GPT2BPETokenizer(AbstractTokenizer):
  ...
  def __init__(self, vocab_file, merge_file):
    ...
    self.tokenizer = GPT2Tokenizer(vocab_file,
                                   merge_file,
                                   errors='replace',
                                   special_tokens=[],
                                   max_len=None)
    self.eod_id = self.tokenizer.encoder['<|endoftext|>']
    # self.ans_id = self.tokenizer.encoder['<ans>']
...
class _AutoTokenizer(AbstractTokenizer):
  ...
  def __init__(self, tokenizer_name_or_path, vocab_extra_ids):
    ...
    self.ans_id = None  # self.tokenizer.encoder['<ans>']
  ...
  @property
  def ans(self):
    if self.ans_id is None:
      ans_ids = self.tokenizer.encode('<ans>')
      assert len(ans_ids) == 1, ans_ids
      self.ans_id = ans_ids[0]
    return self.ans_id

  # @property
  # def pad(self):
  #     if self.pad_id is None:
  #         pad_ids = self.tokenizer.encode('<pad>')
  #         assert len(pad_ids) == 1, pad_ids
  #         self.pad_id = pad_ids[0]
  #     return self.pad_id
