...
def cyclic_iter(iter):
  while True:
    for x in iter:
      yield x
    print_rank_0('current dataset epoch closed')
...
def build_train_valid_test_data_iterators(
  ...
  if mpu.get_tensor_model_parallel_rank() == 0:
    ...
    if not args.valid_data_path:
      ...
      args.eval_iters = math.ceil(len(valid_ds)/args.global_batch_size)
      print_rank_0(f'reset args.eval_iters to : {args.eval_iters}')

      if valid_ds is not None:
        valid_dataloaders = [
            build_pretraining_data_loader(valid_ds, 0, num_workers=args.valid_num_workers,collate_fn=collate_fn)
            ]
      else:
        valid_dataloaders = []
