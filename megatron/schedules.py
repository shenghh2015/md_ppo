...
from megatron import get_args,print_with_rank
...
def forward_step(forward_step_func, data_iterator, model, input_tensor,
                 losses_reduced, num_microbatches=None):
...
def backward_step(optimizer,
  ...
  if args.deepspeed:
    ...
    # Backward pass.
    origin_output_tensor = output_tensor
    ...
    # if mpu.is_pipeline_last_stage():
    #   print_with_rank('backwards',output_tensor.shape,output_tensor_grad is None,output_tensor)
    #   unwrapped_model = unwrap_model(model, (torchDDP, LocalDDP, Float16Module))
    #   print_with_rank('v_head_grad in backwards',unwrapped_model.v_head[2].weight.grad)
    #   print_with_rank('origin_output_tensor',origin_output_tensor)
...
def forward_backward_no_pipelining(forward_step_func, data_iterator, model,
                                   optimizer, timers, forward_only, num_microbatches=None):
...
def forward_backward_pipelining_without_interleaving(forward_step_func,
                                                     data_iterator, model,
                                                     optimizer, timers,
                                                     forward_only, num_microbatches=None):
  ...
  # print_with_rank('num_microbatches,num_warmup_microbatches',num_microbatches,num_warmup_microbatches)

  # Run warmup forward passes.
  for i in range(num_warmup_microbatches):
    input_tensor = p2p_communication.recv_forward(timers)
    output_tensor = forward_step(forward_step_func, data_iterator, model,
                                 input_tensor, losses_reduced, num_microbatches=num_microbatches)

    p2p_communication.send_forward(output_tensor, timers)
    # print_with_rank(f'发送tensor: {output_tensor.shape} 到 rank: {mpu.get_pipeline_model_parallel_next_rank()}')

    input_tensors.append(input_tensor)
    output_tensors.append(output_tensor)

  
  # print_with_rank('开始F1B1',num_warmup_microbatches,num_microbatches,num_microbatches_remaining)
  # Before running 1F1B, need to receive first forward tensor.
  # If all microbatches are run in warmup / cooldown phase, then no need to
  # receive this tensor here.
  if num_microbatches_remaining > 0:
    # print_with_rank(f'准备接受上一个rank: {mpu.get_pipeline_model_parallel_prev_rank()} 的tensor')
    input_tensor = p2p_communication.recv_forward(timers)
    # print_with_rank('接受到tensor')
  ...
  if not forward_only:
    for i in range(num_warmup_microbatches):
      ...
      # import time
      # time.sleep(1)
      # print_with_rank(f'从rank: {mpu.get_pipeline_model_parallel_next_rank()} 接受梯度')
  ...
  # print_with_rank('离开F1B1',num_warmup_microbatches,num_microbatches,num_microbatches_remaining)
