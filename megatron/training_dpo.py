...
def dpo_train(train_valid_test_dataset_provider,
             model_provider,
             forward_step_func,
  ...
  model, optimizer, lr_scheduler = setup_model_and_optimizer(
    model_provider,param_groups_provider=param_groups_provider)
  args.parameters_in_billions_no_embedding = get_parameters_in_billions(
      model, exclude_embeddings=True)
  print_rank_0(
      f'> estimated model parameters: {get_parameters_in_billions(model)}')
  print_rank_0(
  ...
  if args.virtual_pipeline_model_parallel_size is not None:
    all_data_iterators = [
        build_train_valid_test_data_iterators(
            train_valid_test_dataset_provider,collate_fn=collate_fn) for _ in range(len(model))
    ]
    train_data_iterator = [
    ...
    train_data_iterator, valid_data_iterator, test_data_iterator = build_train_valid_test_data_iterators(
        train_valid_test_dataset_provider,collate_fn=collate_fn)
  ...
  elif args.train_weighted_split_paths is not None and len(
      args.train_weighted_split_paths[0]) > 1:
    paths = args.train_weighted_split_paths[0]
    weights = args.train_weighted_split_weights[0]
    data_prefix = [
        j for i in [[w, p] for w, p in zip(weights, paths)] for j in i
    ]
  ...
  args.use_ptx = args.ptx_dataset_path is not None

  # 预训练数据集构造
  if args.use_ptx:
    ptx_dataset = ptx_dataset_provider()
    # micro_batches = get_num_microbatches() 
    ptx_sample_num = args.train_iters * get_num_microbatches() * args.micro_batch_size
    ...
    ptx_dataloader = iter(torch.utils.data.DataLoader(
      ptx_dataset,
  ...
  #   names = names if names is not None else ['valid'
  #                                            ] * len(valid_data_iterator)
...
def get_model(model_provider_func):
  ...
  if mpu.get_pipeline_model_parallel_world_size() > 1 and \
    ...
    # first stage主要负责embedding层
    pre_process = mpu.is_pipeline_first_stage()
    # post stage负责transformer之后的层,包括loss等
    post_process = mpu.is_pipeline_last_stage()
  ...
  if args.DDP_impl == 'local':
    model = [
        LocalDDP(model_module, args.accumulate_allreduce_grads_in_fp32,
                 args.use_contiguous_buffers_in_ddp) for model_module in model
    ]
    for model_module in model:
...
def setup_model_and_optimizer(model_provider_func,param_groups_provider=None,learning_rate_scheduler_provider=None):
  """Setup model and optimizer."""
  args = get_args()
  if learning_rate_scheduler_provider is  None:
    learning_rate_scheduler_provider = get_learning_rate_scheduler
  ...
  if args.inference:
    ...
    optimizer = get_megatron_optimizer(unwrapped_model,param_groups_provider=param_groups_provider)
    ...
    if optimizer is not None:
      # for param_group in optimizer.param_groups:
      #   print('param group lr with before lr_scheduler',param_group['lr'], len(param_group['params']))
      lr_scheduler = learning_rate_scheduler_provider(optimizer)
    else:
      lr_scheduler = learning_rate_scheduler_provider

  # for param_group in optimizer.param_groups:
  #   print('param group lr with after lr_scheduler',param_group['lr'], len(param_group['params']))
  ...
  if args.deepspeed:
    ...
    if args.cpu_optimizer:
      ...
      from megatron.optimizer import _get_params_for_weight_decay_optimization
      if args.inference:
        param_groups = None
      else:
        param_groups = _get_params_for_weight_decay_optimization(unwrapped_model)
    ...
    # save_checkpoint(4001, [model], optimizer, None)
    # print(sjdalj)
    if args.only_save_dummy_model:
      save_checkpoint_and_time(0,[model],None,None)
      # # import time
      # # print('sleeping')
      # # time.sleep(60)
      torch.distributed.barrier()
    ...
    ) == args.fp16, "megatron fp16 config does not match deepspeed"
    assert model.bfloat16_enabled(
    ) == args.bf16, "megatron bf16 config does not match deepspeed"
  ...
  if args.load is not None:
    ...
    # for param_group in optimizer.param_groups:
    #   print('param group lr before load_checkpoint',param_group['lr'], len(param_group['params']))
    # print('strict',not args.load_no_strict)
    args.iteration = load_checkpoint(model, optimizer, lr_scheduler,strict=not args.load_no_strict)
    
    # for param_group in optimizer.param_groups:
    #   print('param group lr after load_checkpoint',param_group['lr'], len(param_group['params']))
    # print_rank_0(model[0].module)
    # for n, v in model[0].named_parameters():
    #   if 'word_embeddings.weight' in n:
    #     print(n, v[:5, :5])
    #     break

    # print_rank_0('word embedding',model[0].module.tied_modules.embed.word_embeddings.weight[:5,:5])
    torch.distributed.barrier()
  ...
  dump_weights(f'{args.universal_checkpoint}', args.iteration, model,
               optimizer)
...
def train_step(forward_step_func, data_iterator, model, optimizer,
               lr_scheduler,ptx_dataloader=None,ptx_forward_step_func=None):
  ...
  if args.deepspeed:
    ...
    # print_rank_0(f'{type(data_iterator)},{len(data_iterator)}')
    # b = next(data_iterator)
    # for k,v in b.items():
    #     print_rank_0(f"k: {k},v: {type(v)},v.shape: {v.shape}")
    # print_rank_0(type(b))
    # print(sdg)
  ...
  # ptx train
  if ptx_dataloader is not None:
    ptx_losses_reduced = forward_backward_func(
      ptx_forward_step_func,
      ptx_dataloader,
      model,
      optimizer,
      timers,
  ...
  if args.deepspeed:
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
      ...
      # 加入预训练的loss
      if ptx_losses_reduced is not None:
        for key in ptx_losses_reduced[0]:
          losses_reduced_for_key = [x[key] for x in ptx_losses_reduced]
          loss_reduced[key] = sum(losses_reduced_for_key) / len(
              losses_reduced_for_key)
...
def training_log(loss_dict,
  ...
  if writer and (iteration % args.tensorboard_log_interval == 0) and \
    ...
    for key in loss_dict:
      ...
      writer.add_scalar(
          f"lm-loss-training/{key}" + ' vs gigaflos (without embeddings)',
          loss_dict[key], args.gigaflos_no_embeds)
    ...
    if args.curriculum_learning:
      writer.add_scalar('curriculum_seqlen', args.curriculum_seqlen, iteration)
  ...
  if iteration % args.log_interval == 0:
    ...
    flops_per_iteration = (
        coefficient * checkpoint_activations_factor * batch_size * seq_len *
        num_layers * (hidden_size**2)) * (1. + (seq_len / (6. * hidden_size)) +
                                          (vocab_size /
                                           (16. * num_layers * hidden_size)))
    tflops = flops_per_iteration / (elapsed_time_per_iteration *
                                    args.world_size * (10**12))
...
def save_checkpoint_and_time(iteration,
                             model,
                             optimizer,
                             lr_scheduler,
                             tag=None):
  ...
  save_checkpoint(iteration, model, optimizer, lr_scheduler, tag=tag)
  torch.distributed.barrier()
...
def train(forward_step_func, model, optimizer, lr_scheduler,
          train_data_iterator, valid_data_iterator,ptx_dataloader=None,ptx_forward_step_func=None):
  """Train the model function."""
  args = get_args()
  timers = get_timers()
  timed_save_start = time.time()
  timed_saving_second = args.timed_saving_second

  if args.rank == 0:
    print(
        "Number of parameters: [tensor rank - pipeline rank] w/ and w/o embeddings:"
    )
  torch.distributed.barrier()
  if mpu.get_data_parallel_rank() == 0:
    tp_rank = mpu.get_tensor_model_parallel_rank()
  ...
  # print_rank_0('word embedding in start training',model[0].module.tied_modules.embed.word_embeddings.weight[:5,:5])
  ...
  while iteration < args.train_iters:
    if (
        ...
        and args.skip_train_iteration_range[0][0] <= iteration + 1 <=
        args.skip_train_iteration_range[0][1]):
    ...
    if args.deepspeed:
      ...
      global_batch_size = mpu.get_data_parallel_world_size() * \
                          args.micro_batch_size * \
                          get_num_microbatches()
      # print_rank_0(f"data_parallel_world_size: {mpu.get_data_parallel_world_size()},args.micro_batch_size: {args.micro_batch_size}, num_microbatches: {get_num_microbatches()}")

      # print(len(model),model.__class__.__name__)
    ...
    args.gigaflos_no_embeds += (
        6 * new_samples * args.seq_length *
        get_parameters_in_billions(model, exclude_embeddings=True))
    ...
    # timed save checkpoint
    # add by zx
    if args.save and timed_saving_second > 0:
      cur_time = time.time()
      if cur_time - timed_save_start > timed_saving_second:
        print_rank_0(
            f'timed saving checkpoint for every {timed_saving_second} seconds')
        save_checkpoint_and_time(iteration,
                                 model,
                                 optimizer,
                                 lr_scheduler,
                                 tag='timed_save_checkpoint')
        timed_save_start = cur_time
...
def evaluate(forward_step_func, data_iterator, model, verbose=False):
  ...
  with torch.no_grad():
    ...
    while iteration < args.eval_iters:
      ...
      loss_dicts_list.extend(loss_dicts)

      # if mpu.is_pipeline_last_stage(ignore_virtual=True):
      #   # Reduce across processes.
      #   # eval loss
      #   eval_loss = [loss_dict['loss'] for loss_dict in loss_dicts]
      #   chosen_end_scores = sum([loss_dict['chosen_end_scores'] for loss_dict in loss_dicts],[])
      #   reject_end_scores = sum([loss_dict['reject_end_scores'] for loss_dict in loss_dicts],[])
  ...
  # 计算指标
  if mpu.is_pipeline_last_stage(ignore_virtual=True):
    # print(loss_dicts_list[0])
    eval_loss = sum([loss_dict['loss'] for loss_dict in loss_dicts_list])/len(loss_dicts_list)
    chosen_end_scores = sum([loss_dict['chosen_end_scores'] for loss_dict in loss_dicts_list],[])
    reject_end_scores = sum([loss_dict['reject_end_scores'] for loss_dict in loss_dicts_list],[])
    # 计算 acc
    acc = sum(
      [1 if chosen_end_score > reject_end_score else 0 for chosen_end_score, reject_end_score in zip(chosen_end_scores,reject_end_scores)]
      )/len(chosen_end_scores)
    
    total_loss_dict['loss'] = eval_loss
    total_loss_dict['acc'] = acc
  ...
  for key in total_loss_dict:
    ...
    if writer and is_last_rank():
      ...
      writer.add_scalar(f'{tf_plot_prefix}/{key} validation vs tokens',
                        ...
                        args.consumed_train_tokens)
      writer.add_scalar(
          f'{tf_plot_prefix}/{key} validation vs gigaflos (without embeddings)',
          total_loss_dict[key], args.gigaflos_no_embeds)
      ...
      #   writer.add_scalar(
      #       f'{tf_plot_prefix}/{key} validation ppl vs gigaflos (without embeddings)',
...
def build_train_valid_test_data_iterators(
    build_train_valid_test_datasets_provider,collate_fn=None
    ):
  """XXX"""
  args = get_args()
  ...
  # print_with_rank(f'enter dataloader')
  ...
  if mpu.get_tensor_model_parallel_rank() == 0:
    ...
    # print_with_rank(f'datasetsfgsfgdgh: {type(train_ds)}')
    ...
    # by a few percent
    valid_dataloaders = [build_pretraining_data_loader(
      d, args.consumed_valid_samples // len(valid_ds), 
    ...
    test_dataloaders = [build_pretraining_data_loader(d, 0,collate_fn=collate_fn) for d in test_ds] \
    ...
    flags = torch.cuda.LongTensor([
        int(do_train),
        len(valid_dataloaders) if args.eval_iters > 0 else
        0,  # eval_iters == 0 is equivalent to having no validation
        len(test_dataloaders) if args.eval_iters > 0 else
