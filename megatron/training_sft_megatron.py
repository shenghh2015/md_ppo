...
from megatron import mpu,torch_save_helper
...
def pretrain(train_valid_test_dataset_provider,
  ...
  assert not args.deepspeed, "this training not support deepspeed"
...
def setup_model_and_optimizer(model_provider_func,param_groups_provider=None,learning_rate_scheduler_provider=None):
  ...
  if args.inference or args.only_save_dummy_model:
    optimizer = None
    lr_scheduler = None
  else:
...
def save_checkpoint_and_time(iteration,
  ...
  def _save_checkpoint_and_time():
    save_checkpoint(iteration, model, optimizer, lr_scheduler, tag=tag)

  args = get_args()
  origin_torch_save_fn = torch.save
  if args.save_with_less_memory:
    torch.save = partial(torch_save_helper,origin_torch_save_fn)

  _save_checkpoint_and_time()

  torch.save = origin_torch_save_fn
  ...
  for key in total_loss_dict:
    ...
    if writer and is_last_rank():
      ...
      writer.add_scalar(
          ...
          total_loss_dict[key].item(), args.gigaflos_no_embeds)
...
def build_train_valid_test_data_iterators(
  ...
  if mpu.get_tensor_model_parallel_rank() == 0:
    ...
    print_rank_0('    validation sample num: {}'.format(len(valid_ds)))
    print_rank_0('    test sample num:       {}'.format(len(test_ds)))
    ...
    test_dataloaders = [build_pretraining_data_loader(
      test_ds,
      0,
      collate_fn=collate_fn,
    )]
    ...
    args.eval_iters = math.ceil(len(valid_dataloaders[0])/args.global_batch_size)

    print_rank_0(f'reset args.eval_iters to : {args.eval_iters }')
    

    print_rank_0('    train dataloader total batch:      {}'.format(len(train_dataloader)))
    print_rank_0('    valid dataloader total batch:      {}'.format(len(valid_dataloaders[0])))
  ...
  if valid_dataloaders is not None:
    valid_data_iterators = [iter(cyclic_iter(valid_dataloaders[0]))]
                             
  else:
    valid_data_iterators = [None] * num_valid_ds

  if test_dataloaders is not None:
    test_data_iterators = [iter(cyclic_iter(test_dataloaders[0]))]
