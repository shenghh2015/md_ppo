...
def best_fitting_sft_dtype(vocab_size=None):
  return np.int32 
...
class IndexedDatasetBuilder(object):
  ...
  def finalize(self, index_file):
    ...
    IndexedDatasetBuilder.write_header(index, self.dtype,
                                       len(self.data_offsets), len(self.sizes),
                                       len(self.doc_idx))
    ...
    def writer(cls, path, dtype):
      ...
      class _Writer(object):
        ...
        def _get_pointers(sizes, npdtype):
          ...
          pointers, _ = get_pointers_with_total(sizes,
                                                dtype().itemsize, npdtype)
          return pointers

        def write(self, sizes, doc_idx):
          MMapIndexedDataset.Index.write_header(self._file, dtype, len(sizes),
                                                len(doc_idx))
...
def gather_files_dist_check_dtype(filelist, dtype_rank_consistent, dtype_code,
                                  distctx):
  ...
  distctx.allassert(dtype_rank_consistent,
                    "Some rank found inconsistent dtype values")
  ...
  distctx.allassert(dtype_code == first_dtype_code or dtype_code is None,
...
def gather_files_dist_idx_cached(outfile, filelist, distctx):
  ...
  dtype = gather_files_dist_check_dtype(filelist, dtype_rank_consistent,
                                        dtype_value, distctx)
  ...
  try:
    ...
    with distctx.open(finalnametmp) as fout:
      ...
      if distctx.rank == 0:
        try:
          ...
          file_offset += IndexedDatasetBuilder.write_header(
              fout, dtype, global_data_count, global_size_count,
              global_doc_count)
        except Exception as e:
          err = e
      distctx.allraise_if(err)
      ...
      write_list_at_offset(fout, file_offset, dim_offsets, global_size_offset,
                           global_dim_offset, np.int64)
      ...
      write_list_at_offset(fout, file_offset, data_offsets, data_shift,
                           global_data_offset, np.int64)
      ...
      write_list_at_offset(fout, file_offset, sizes, 0, global_size_offset,
                           np.int64)
      ...
      write_list_at_offset(fout, file_offset, doc_idx, global_size_offset,
                           global_doc_offset, np.int64)
...
def gather_files_dist_idx_mmap(outfile, filelist, distctx):
  ...
  pointers, pointers_bytes = get_pointers_with_total(sizes,
                                                     dtype().itemsize,
                                                     np.int64)
  ...
  try:
    ...
    with distctx.open(finalnametmp) as fout:
      ...
      write_list_at_offset(fout, file_offset, pointers, pointer_offset,
                           global_size_offset, np.int64)
      file_offset += global_size_count * np.int64().itemsize
...
def gather_files_dist_check_impltype(filelist, distctx):
  ...
  matchtype = sametype and (ourtype is None
                            or ourtype == bcasttype) and bcasttype != 0
