  ...
  # Blending dataset.
    ...
    for i in range(len(prefixes)):
      train_ds, valid_ds, test_ds = _build_train_valid_test_datasets(
          ...
          pad_token=pad_token,
          eos_token=eos_token,
          train_valid_test_num_samples=datasets_train_valid_test_num_samples[
              i],
          seed=seed,
          skip_warmup=skip_warmup)
      if train_ds:
...
def build_dataset_group(dataset_group_name, paths, weights, splits, data_impl,
                        seq_length: int, pad_token: int, eos_token: int,
                        train_valid_test_num_samples, seed, skip_warmup,
                        train_valid_test):
  '''
...
def _build_single_datasets(data_prefix, range_string, data_impl,
  ...
  target_indexed_dataset = get_indexed_dataset(data_prefix=data_prefix,
                                               is_input=False,
                                               data_impl=data_impl,
                                               skip_warmup=skip_warmup)
  ...
  splits = get_split_by_range_(range_string=range_string,
                               size=total_num_of_documents)
  ...
  print_rank_0('     document indices in [{}, {}) total of {} '
               'documents'.format(splits[0], splits[1], splits[1] - splits[0]))

  def build_dataset(name):
    dataset = None
    if splits[1] > splits[0]:
      documents = np.arange(start=splits[0],
                            stop=splits[1],
                            step=1,
                            dtype=np.int32)
...
def _build_train_valid_test_datasets(data_prefix, data_impl, splits_string,
  ...
  # Target indexed dataset.
  target_indexed_dataset = get_indexed_dataset(data_prefix,
                                               is_input=False,
                                               data_impl=data_impl,
                                               skip_warmup=skip_warmup)
...
class DecoderPackedMTFDataset(torch.utils.data.Dataset):
  ...
  def __init__(
    ...
    self.mtf_dataset = MTFDataset(name=name,
                                  data_prefix=data_prefix,
                                  data_impl=data_impl,
                                  skip_warmup=skip_warmup,
                                  documents=documents)
  ...
  def pack_samples(self, items):
    ...
    decoder_tokens = np.full((self.seq_length, ),
                             self.pad_token,
                             dtype=np.int64)
    ...
    for token_dict in items:
      ...
      if cur_len + total_len > self.seq_length:
        ...
        raise ValueError(
            f"""Items to be packed do not fit inside a single sample.
      ...
      decoder_tokens[cur_len:cur_len +
                     input_token_len] = token_dict["input_tokens"]
      decoder_tokens[cur_len + input_token_len:cur_len +
                     total_len] = token_dict["target_tokens"]
...
def _build_index_mappings(
  ...
  if torch.distributed.get_rank() == 0:
    if (not os.path.isfile(sample_idx_filename)) or \
      ...
      while len(sample_idx) <= num_samples:
        new_document_ids = _build_shuffle_idx(nb_documents=nb_documents,
                                              np_rng=np_rng)
      ...
      print_rank_0(
          ' > elasped time to build and save shuffle-idx and sample-idx mapping'
  ...
  start_time = time.time()
  print_rank_0(
      ' > loading doc-idx mapping from {}'.format(sample_idx_filename))
  ...
  for current_sample_end, document_id in enumerate(document_ids):
    ...
    if row_length > seq_length:
      ...
      if current_sample_end > current_sample_start:
        full_samples.append(
            np.asarray([current_sample_start, current_sample_end]))
      ...
      if tok_len > seq_length:
        ...
        logger.warning(
            f"Skipping sample id={document_id}. Maximum sequence length: {seq_length}, sample length: {tok_len}"
        )
...
def get_indexed_dataset(data_prefix: str, is_input: bool, data_impl: str,
                        skip_warmup: bool):
  if is_input:
    field = "inputs"
  else:
  ...
  return get_indexed_dataset_(f"{data_prefix}_{field}_document", data_impl,
                              skip_warmup)
...
def get_indexed_dataset_(path, data_impl, skip_warmup):
  ...
  start_time = time.time()
  indexed_dataset = make_indexed_dataset(path, data_impl, skip_warmup)
