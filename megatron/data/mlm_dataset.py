...
def build_train_valid_test_datasets(data_prefix, data_impl, splits_string,
                                    ...
                                    sequence_length, noise_density,
                                    mean_noise_span_length, seed, skip_warmup):
...
def build_dataset_group(dataset_group_name, paths, weights, splits, data_impl,
                        ...
                        noise_density, mean_noise_span_length, seed,
...
def _build_single_datasets(data_prefix, range_string, data_impl,
                           train_valid_test_num_samples, sequence_length,
                           noise_density, mean_noise_span_length, seed,
                           skip_warmup, dataset_group_name, train_valid_test):
...
class MLMDataset(torch.utils.data.Dataset):
  ...
  def __init__(self,
    ...
    assert len(
        self.sentinel_token_ids
    ) >= self.num_noise_spans, "Not enough sentinel tokens, please add more"
...
def build_training_sample(
  ...
  spans_end = np.concatenate(
      [spans_start[1:],
       np.full((1, ), len(sample), dtype=np.int32)])

  sentinel_token_ids = all_sentinel_token_ids[:num_noise_spans]

  input_token_ids = np.concatenate([
      elt for start, end, sentinel_token in zip(
          spans_start[::2], spans_end[::2], sentinel_token_ids) for elt in
      [sample[start:end],
       np.full((1, ), sentinel_token, dtype=np.int32)]
  ] + [np.full((1, ), sep_id, dtype=np.int32)])
  target_token_ids = np.concatenate([
      elt for start, end, sentinel_token in zip(
          spans_start[1::2], spans_end[1::2], sentinel_token_ids) for elt in
      [np.full((1, ), sentinel_token, dtype=np.int32), sample[start:end]]
  ] + [np.full((1, ), sep_id, dtype=np.int32)])

  return {'input_tokens': input_token_ids, 'target_tokens': target_token_ids}


def compute_input_and_target_lengths(sequence_length, noise_density,
  ...
  inputs_length, targets_length, num_noise_spans = _tokens_length_to_inputs_length_targets_length(
      tokens_length)
...
def random_spans_noise_mask(
  ...
  span_starts = np.concatenate([
      np.full((1, ), 0, dtype=np.int32),
      np.cumsum(interleaved_span_lengths)[:-1]
  ])
