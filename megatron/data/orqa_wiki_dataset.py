...
def get_open_retrieval_wiki_dataset():
  ...
  dataset = OpenRetrievalEvidenceDataset('2018 Wikipedia from DPR codebase',
                                         ...
                                         tokenizer, args.retriever_seq_length)
  return dataset
...
def get_open_retrieval_batch(data_iterator):
  ...
  keys = [
      'row_id', 'context', 'context_mask', 'context_types', 'context_pad_mask'
  ]
...
def build_tokens_types_paddings_from_ids(text_ids, max_seq_length, cls_id,
                                         sep_id, pad_id):
  """Build token types and paddings, trim if needed, and pad if needed."""
  enc_ids = []
  tokentypes_enc = []
...
class OpenRetrievalEvidenceDataset(ABC, Dataset):
  ...
  def __getitem__(self, idx):
    ...
    sample = build_sample(row['doc_id'], context_ids, context_types,
                          context_pad_mask)
    return sample
