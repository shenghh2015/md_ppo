...
class MTFDataset(torch.utils.data.Dataset):
  ...
  def __init__(
    ...
    # Dataset.
    self.input_indexed_dataset = get_indexed_dataset(data_prefix,
                                                     is_input=True,
                                                     data_impl=data_impl,
                                                     skip_warmup=skip_warmup)
    self.target_indexed_dataset = get_indexed_dataset(data_prefix,
                                                      is_input=False,
                                                      data_impl=data_impl,
                                                      skip_warmup=skip_warmup)
    ...
    assert self.input_indexed_dataset.sizes.shape[
        0] == self.target_indexed_dataset.sizes.shape[0]
