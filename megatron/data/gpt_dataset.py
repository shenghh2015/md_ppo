...
def build_train_valid_test_datasets(data_prefix, data_impl, splits_string,
  ...
  # print_rank_0(f'data_prefix: {data_prefix}')
  # print(sdg)
  # Single dataset.
  if len(data_prefix) == 1:
    all_train_datasets, all_valid_datasets, all_test_datasets = _build_train_valid_test_datasets(
        data_prefix[0], data_impl, splits_string, train_valid_test_num_samples,
  ...
  # Blending dataset.
    ...
    for i in range(len(prefixes)):
      train_ds, valid_ds, test_ds = _build_train_valid_test_datasets(
          ...
          datasets_train_valid_test_num_samples[i], seq_length, seed,
          skip_warmup)
      if train_ds:
...
def build_dataset_group(dataset_group_name, paths, weights, splits, data_impl,
  ...
  # Single dataset.
  if len(paths) == 1:
    dataset = _build_single_datasets(paths[0], splits[0], data_impl,
                                     train_valid_test_num_samples, seq_length,
                                     seed, skip_warmup, dataset_group_name,
                                     train_valid_test)
    return dataset
  # Blending dataset.
  else:
    ...
    datasets = []
    for i in range(len(prefixes)):
      ds = _build_single_datasets(prefixes[i], splits[i], data_impl,
...
class GPTDataset(torch.utils.data.Dataset):
  ...
  def __init__(self, name, data_prefix, documents, indexed_dataset,
    ...
    """
        documents: 文档的序号
        num_samples: 总共需要采样的样本数量
        seq_length: 最大长度,用来做padding
        """
    ...
    # print_rank_0(，len(self.indexed_dataset.sizes))
    # print_rank_0(type(self.indexed_dataset.sizes))
    # print_rank_0(self.indexed_dataset.sizes.shape)
    # print_rank_0(self.indexed_dataset.sizes[:10])
    print_rank_0(
        f'mean token num in all documents: {self.indexed_dataset.sizes.mean()}'
    )
...
def _build_index_mappings(name,
  ...
  # print(f'sdgsgdfsfgsfgs------{len(documents)}, {tokens_per_epoch}')
  num_epochs = _num_epochs(tokens_per_epoch, seq_length, num_samples)
  true_num_epochs = seq_length * num_samples / tokens_per_epoch
  # print_rank_0(f'totol epoch num is:{}')
  # rng state
  ...
  print_rank_0('    total number of epochs: {}'.format(true_num_epochs))
