...
def parse_args(extra_args_provider=None,
  ...
  parser = _add_compile_args(parser)
  parser = _add_debug_args(parser)
  # add lora args
  parser = _add_lora_args(parser)
  # add ntk args
  parser = _add_ntk_args(parser)
  # Custom arguments.
  ...
  if args.global_batch_size is None:
    ...
    if args.rank == 0:
      print('setting global batch size to {}'.format(args.global_batch_size),
            flush=True)
  ...
  if args.custom_layer_split is not None:
    assert sum(args.custom_layer_split) == args.num_layers, args.custom_layer_split
    assert len(args.custom_layer_split) == args.pipeline_model_parallel_size,  args.custom_layer_split
    ...
    for range_ in args.skip_train_iteration_range:
      if len(range_) == 2:
        ...
        if not skip_train_iteration_range:
          ...
          skip_train_iteration_range[-1][1] = max(
              end, skip_train_iteration_range[-1][1])
        else:
          skip_train_iteration_range.append([start, end])
      else:
        raise ValueError(
    ...
    if args.log_path is not None:
      with open(
          os.path.join(args.log_path,
                       f'args_{time.strftime("%Y-%m-%dT%H:%M:%S")}.txt'),
          'w') as f:
...
def _add_network_size_args(parser):
  ...
  group.add_argument('--num-layers',
                     type=int,
                     default=None,
                     help='Number of transformer layers.')
  group.add_argument('--custom-layer-split',
                     type=lambda x :eval(x),
                     default=None,
                     help='custom layer split, if none, uniform is considered')
  ...
  def _list_type(s):
    ...
    return r
  group.add_argument('--custom-deepspeed-layer-split',
                     type=_list_type,
                     default=None,
                     help='custom layer split, if none, uniform is considered'      
                     )
  ...
  group.add_argument('--ffn-hidden-size',
                     type=int,
                     default=None,
                     help='Transformer Feed-Forward Network hidden size. '
                     'This is set to 4*hidden-size if not provided')
  group.add_argument('--num-attention-heads',
                     type=int,
                     default=None,
                     help='Number of transformer attention heads.')
  group.add_argument('--kv-channels',
                     type=int,
                     default=None,
                     ...
                     'if not provided.')
  group.add_argument('--max-position-embeddings',
                     type=int,
                     default=None,
  ...
  group.add_argument('--layernorm-epsilon',
                     type=float,
                     default=1e-5,
  ...
  group.add_argument('--openai-gelu',
                     action='store_true',
                     help='Use OpenAIs GeLU implementation. This option'
                     'should not be used unless for backward compatibility'
                     'reasons.')
  group.add_argument('--onnx-safe',
                     type=bool,
                     required=False,
                     help='Use workarounds for known problems with '
                     'Torch ONNX exporter')
  group.add_argument('--bert-no-binary-head',
                     action='store_false',
  ...
  group.add_argument(
      ...
      help=
      'Define position embedding type ("absolute" | "rotary" | "alibi"). "absolute" by default.'
  )
  group.add_argument(
    '--alibi-micro-batch-size',
    default=None,
    type=int,
    help='batch size for alibi batch size, defualt is micro-batch-size'
  )
  group.add_argument(
      '--glu-activation',
      type=str,
  ...
  group.add_argument(
      ...
      type=str,
      help=
      'path to look for a kill switch, which if found will automatically exit the program'
  )
...
def _add_logging_args(parser):
  ...
  group.add_argument('--log-params-norm',
                     action='store_true',
  ...
  group.add_argument('--tensorboard-log-interval',
                     type=int,
                     default=1,
                     help='Report to tensorboard interval.')
  group.add_argument('--tensorboard-queue-size',
                     type=int,
                     default=1000,
                     help='Size of the tensorboard queue for pending events '
                     'and summaries before one of the ‘add’ calls forces a '
                     'flush to disk.')
  group.add_argument('--log-timers-to-tensorboard',
                     action='store_true',
                     help='If set, write timers to tensorboard.')
  group.add_argument('--log-batch-size-to-tensorboard',
                     action='store_true',
...
def _add_regularization_args(parser):
  ...
  group.add_argument('--attention-dropout',
                     type=float,
                     default=0.1,
                     help='Post attention dropout probability.')
  group.add_argument('--hidden-dropout',
                     type=float,
                     default=0.1,
  ...
  group.add_argument('--adam-eps',
                     ...
                     'numerical stability')
  group.add_argument('--sgd-momentum',
                     type=float,
                     default=0.9,
...
def _add_training_args(parser):
  ...
  group.add_argument('--variable-seq-lengths',
                    action='store_true',
                    help="if variable seq lengths, if true, some communication overhead"
  )
  ...
  group.add_argument('--global-batch-size',
                     type=int,
                     default=None,
  ...
  group.add_argument('--rampup-batch-size',
                     nargs='*',
                     default=None,
  ...
  group.add_argument('--checkpoint-num-layers',
                     type=int,
                     default=1,
                     help='chunk size (number of layers) for checkpointing.')
  group.add_argument('--train-iters',
                     type=int,
                     default=None,
                     help='Total number of iterations to train over all '
                     'training runs. Note that either train-iters or '
                     'train-samples should be provided.')
  group.add_argument('--train-samples',
                     type=int,
                     default=None,
                     help='Total number of samples to train over all '
                     'training runs. Note that either train-iters or '
                     'train-samples should be provided.')
  group.add_argument('--train-tokens',
                     type=int,
                     default=None,
  ...
  group.add_argument('--exit-interval',
                     type=int,
                     default=None,
                     help='Exit the program after the iteration is divisible '
                     'by this value.')
  group.add_argument('--exit-duration-in-mins',
                     type=int,
                     default=None,
                     help='Exit the program after this many minutes.')
  group.add_argument('--tensorboard-dir',
                     type=str,
                     default=None,
  ...
  group.add_argument('--no-bias-gelu-fusion',
                     action='store_false',
                     help='Disable bias and gelu fusion.',
                     dest='bias_gelu_fusion')
  group.add_argument('--no-bias-dropout-fusion',
                     action='store_false',
  ...
  group.add_argument(
    ...
    type=int,
    default=4096,
    help="min 8it size, parameter tensors  will be quantilize if elements is great than the given threshold"
  )
  group.add_argument('--dataloader-type',
                     type=str,
                     default=None,
  ...
  group.add_argument('--cpu_torch_adam',
                     action='store_true',
                     help='Use Torch Adam as optimizer on CPU.')
  group.add_argument('--codecarbon-dir',
                     type=str,
                     default=None,
  ...
  group.add_argument(
      ...
      default=None,
      help=
      'Iteration ranges to skip. The values are one or more dash-separated ranges. e.g., 101-200 251-300.'
  )
  group.add_argument(
      '--inference',
      action='store_true',
  ...
  group.add_argument(
      ...
      action='store_true',
      help=
      "If set to True, the program will abort if the constraints for loading a fused kernel aren't met"
  )
  group.add_argument(
      '--pp-partition-method',
      type=str,
      default=None,
      help=
      "Use to override the pipeline stages partitioning method. e.g., 'type:transformer|embedding'"
  )
...
def _add_initialization_args(parser):
  ...
  group.add_argument('--seed',
                     ...
                     'pytorch, and cuda.')
  group.add_argument('--init-method-std',
                     type=float,
                     default=0.02,
                     help='Standard deviation of the zero mean normal '
                     'distribution used for weight initialization.')
  group.add_argument('--init-method-xavier-uniform',
                     action='store_true',
...
def _add_learning_rate_args(parser):
  ...
  group.add_argument('--lr-decay-samples',
                     type=int,
                     default=None,
                     help='number of samples to decay learning rate over,'
                     ' If None defaults to `--train-samples`')
  group.add_argument('--lr-decay-tokens',
                     type=int,
                     default=None,
                     help='number of tokens to decay learning rate over,'
                     ' If not None will override iter/sample-based decay')
  group.add_argument('--lr-warmup-fraction',
                     type=float,
                     default=None,
                     help='fraction of lr-warmup-(iters/samples) to use '
                     'for warmup (as a float)')
  group.add_argument('--lr-warmup-iters',
                     type=int,
                     default=0,
                     help='number of iterations to linearly warmup '
                     'learning rate over.')
  group.add_argument('--lr-warmup-samples',
                     type=int,
                     default=0,
  ...
  group.add_argument('--override-lr-scheduler',
                     action='store_true',
  ...
  group.add_argument('--use-checkpoint-lr-scheduler',
                     action='store_true',
                     help='Use checkpoint to set the values of the scheduler '
                     '(learning rate, warmup iterations, minimum learning '
                     'rate, maximum number of iterations, and decay style '
                     'from checkpoint and ignore input arguments.')
  group.add_argument('--universal-checkpoint',
                     action='store_true',
...
def _add_checkpointing_args(parser):
  ...
  group.add_argument('--save-with-less-memory',
                     action='store_true',
                     help='save without more cpu memory')
  group.add_argument('--save-interval',
                     type=int,
                     default=None,
                     help='Number of iterations between checkpoint saves.')
  group.add_argument('--no-save-optim',
                     action='store_true',
                     default=None,
                     help='Do not save current optimizer.')
  
  # group.add_argument('--no-save-deepspeed-zero-checkpoint',
  #                    action='store_true',
  #                    default=None,
  #                    help='Do not save deepspeed zero checkpoint., include  optimizer state.')
  group.add_argument('--no-save-rng',
                     action='store_true',
                     default=None,
  ...
  group.add_argument('--load-no-strict',
                    action='store_true',
                    help="if load checkpoint state dict strictly"
  )
  group.add_argument('--no-load-optim',
                     action='store_true',
                     default=None,
                     help='Do not load optimizer when loading checkpoint.')
  group.add_argument('--no-load-rng',
                     action='store_true',
                     default=None,
  ...
  group.add_argument('--timed-saving-second',type=int,default=0,\
                     help='timed saving, unit is second')
...
def _add_mixed_precision_args(parser):
  ...
  group.add_argument('--bf16',
                     action='store_true',
                     help='Run model in bfloat16 mode.')
  group.add_argument('--loss-scale',
                     type=float,
                     default=None,
                     help='Static loss scaling, positive power of 2 '
                     'values can improve fp16 convergence. If None, dynamic'
                     'loss scaling is used.')
  group.add_argument('--initial-loss-scale',
                     type=float,
                     default=2**10,
  ...
  group.add_argument('--loss-scale-window',
                     type=float,
                     default=1000,
                     help='Window over which to raise/lower dynamic scale.')
  group.add_argument('--hysteresis',
                     type=int,
                     default=2,
                     help='hysteresis for dynamic loss scaling')
  group.add_argument('--fp32-residual-connection',
                     action='store_true',
                     help='Move residual connections to fp32.')
  group.add_argument('--no-query-key-layer-scaling',
                     action='store_false',
                     help='Do not scale Q * K^T by 1 / layer-number.',
                     dest='apply_query_key_layer_scaling')
  group.add_argument('--attention-softmax-in-fp32',
                     action='store_true',
  ...
  group.add_argument('--fp16-lm-cross-entropy',
                     action='store_true',
...
def _add_distributed_args(parser):
  ...
  group.add_argument('--tensor-model-parallel-size',
                     type=int,
                     default=1,
                     help='Degree of tensor model parallelism.')
  group.add_argument('--pipeline-model-parallel-size',
                     type=int,
                     default=1,
  ...
  group.add_argument('--DDP-impl',
                     default='local',
                     choices=['local', 'torch'],
                     help='which DistributedDataParallel implementation '
                     'to use.')
  group.add_argument('--use-contiguous-buffers-in-ddp',
                     action='store_true',
  ...
  group.add_argument('--lazy-mpu-init',
                     type=bool,
                     required=False,
  ...
  group.add_argument('--use-cpu-initialization',
                     action='store_true',
                     default=None,
                     help='If set, affine parallel weights '
...
def _add_validation_args(parser):
  ...
  group.add_argument('--eval-iters',
                     type=int,
                     default=100,
                     help='Number of iterations to run for evaluation'
                     'validation/test for.')
  group.add_argument('--eval-interval',
                     type=int,
                     default=1000,
...
def _add_data_args(parser):
  ...
  class parse_data_paths_path(argparse.Action):
    ...
    def __call__(self, parser, args, values, option_string=None):
      ...
      with open(values, "r") as fi:
        ...
        assert len(
            lines
        ) == 1, f"Got multiple lines {len(lines)} instead of 1 expected"
        assert lines[0][-2:] == "\"\n" and lines[0][
            0] == "\"", f"Invalid input format, got {lines}"
        values = lines[0][1:-2].split("\" \"")
        weighted_split_paths_dest = re.sub(r"_path$", "", self.dest)
        weighted_split_paths_option = re.sub(r"-path$", "",
  ...
  group.add_argument('--train-weighted-split-paths-path',
                     type=str,
                     action=parse_data_paths_path,
                     default=None)
  group.add_argument('--valid-weighted-split-paths-path',
                     type=str,
                     action=parse_data_paths_path,
                     default=None)
  group.add_argument('--test-weighted-split-paths-path',
                     type=str,
                     action=parse_data_paths_path,
                     default=None)
  ...
  group.add_argument('--vocab-extra-ids',
                     type=int,
                     default=0,
  ...
  group.add_argument('--encoder-seq-length',
                     type=int,
                     default=None,
                     help='Maximum encoder sequence length to process.'
                     'This should be exclusive of --seq-length')
  group.add_argument('--decoder-seq-length',
                     type=int,
                     default=None,
                     help="Maximum decoder sequence length to process.")
  group.add_argument('--retriever-seq-length',
                     type=int,
                     default=256,
  ...
  group.add_argument('--sample-rate',
                     ...
                     ' < sample_rate < 1')
  group.add_argument('--mask-prob',
                     type=float,
                     default=0.15,
                     help='Probability of replacing a token with mask.')
  group.add_argument('--short-seq-prob',
                     type=float,
                     default=0.1,
                     help='Probability of producing a short sequence.')
  group.add_argument('--mmap-warmup',
                     action='store_true',
  ...
  group.add_argument('--valid-num-workers',
                     type=int,
                     default=0,
  ...
  group.add_argument('--data-impl',
                     type=str,
                     default='infer',
                     choices=['lazy', 'cached', 'mmap', 'infer'],
                     help='Implementation of indexed datasets.')
  group.add_argument('--reset-position-ids',
                     action='store_true',
  ...
  group.add_argument('--eod-mask-loss',
                     action='store_true',
                     help='Mask loss for the end of document tokens.')
  group.add_argument('--loss-on-targets-only',
                     action='store_true',
  ...
  group.add_argument(
      ...
      action="store_true",
      help=
      'Some objectives require us to sample loss_mask. This might introduce bias towards '
  ...
  group.add_argument("--noise-density",
                     type=float,
                     default=None,
                     help="Span corruption noise density")
  group.add_argument("--mean-noise-span-length",
                     type=int,
                     default=None,
...
def _add_autoresume_args(parser):
  ...
  group.add_argument('--adlr-autoresume',
                     action='store_true',
                     help='Enable autoresume on adlr cluster.')
  group.add_argument('--adlr-autoresume-interval',
                     type=int,
                     default=1000,
                     help='Intervals over which check for autoresume'
                     'termination signal')
  # # add by zx
  # group.add_argument('--resume_work_path', type=str,
  #                    help='work path to resume from')
...
def _add_biencoder_args(parser):
  ...
  # network size
  group.add_argument('--ict-head-size',
                     type=int,
                     default=None,
                     help='Size of block embeddings to be used in ICT and '
                     'REALM (paper default: 128)')
  group.add_argument('--biencoder-projection-dim',
                     type=int,
                     default=0,
  ...
  # checkpointing
  group.add_argument('--ict-load',
                     type=str,
                     default=None,
                     help='Directory containing an ICTBertModel checkpoint')
  group.add_argument('--bert-load',
                     type=str,
                     default=None,
  ...
  # data
  group.add_argument('--titles-data-path',
                     type=str,
                     default=None,
                     help='Path to titles dataset used for ICT')
  group.add_argument('--query-in-block-prob',
                     type=float,
                     default=0.1,
                     help='Probability of keeping query in block for '
                     'ICT dataset')
  group.add_argument('--use-one-sent-docs',
                     action='store_true',
                     help='Whether to use one sentence documents in ICT')
  group.add_argument('--evidence-data-path',
                     type=str,
                     default=None,
                     help='Path to Wikipedia Evidence frm DPR paper')

  # training
  group.add_argument('--retriever-report-topk-accuracies',
                     nargs='+',
                     type=int,
                     default=[],
                     help="Which top-k accuracies to report "
                     "(e.g. '1 5 20')")
  group.add_argument('--retriever-score-scaling',
                     action='store_true',
                     help='Whether to scale retriever scores by inverse '
                     'square root of hidden size')

  # faiss index
  group.add_argument('--block-data-path',
                     type=str,
                     default=None,
                     help='Where to save/load BlockData to/from')
  group.add_argument('--embedding-path',
                     type=str,
                     default=None,
  ...
  # indexer
  group.add_argument('--indexer-batch-size',
                     type=int,
                     default=128,
                     help='How large of batches to use when doing indexing '
                     'jobs')
  group.add_argument('--indexer-log-interval',
                     type=int,
                     default=1000,
...
def _add_vit_args(parser):
  ...
  group.add_argument('--img-dim',
                     type=int,
                     default=224,
                     help='Image size for vision classification task')
  group.add_argument('--num-channels',
                     type=int,
                     default=3,
                     help='Number of channels in input image data')
  group.add_argument('--patch-dim',
                     type=int,
                     default=16,
...
def _add_zero_args(parser):
  ...
  group.add_argument('--remote-device',
                     type=str,
                     default='none',
...
def _add_memoryopt_args(parser):
  ...
  group.add_argument(
      ...
      default=1,
      help=
      'Make all linear layers the same size of [hidden/tile_factor, hidden/tile_factor]. '
...
def _add_activation_checkpoint_args(parser):
  ...
  group.add_argument('--deepspeed-activation-checkpointing',
                     action='store_true',
...
def _add_compile_args(parser):
  group = parser.add_argument_group('compile', 'cuda or cpu compile')
  group.add_argument('--disable-compile',
                     action='store_true',
                     help='disable to recompile cuda or cpu')

  return parser

def _add_debug_args(parser):
  group = parser.add_argument_group('debug', 'debug args')
  group.add_argument(
    '--only-save-dummy-model',
    action='store_true',
    help='save dummy-model, then kill the program'
  )
  return parser


def _add_lora_args(parser):
  group = parser.add_argument_group('lora_config', 'lora config')
  group.add_argument('--use_lora',
                      action='store_true',
                      help='whether use lora')
  
  group.add_argument('--lora_rank',
                      default=None,
                      type=int,
                      help='lora rank')

  group.add_argument('--lora_alpha',
                    default=None,
                    type=float,
                    help='lora alpha')
  
  group.add_argument('--lora_dropout',
                    default=0.0,
                    type=float,
                    help='lora dropout')
...
def _add_ntk_args(parser):
  group = parser.add_argument_group('ntk_config', 'ntk config')
  group.add_argument('--use_ntk',
                     action='store_true',
                     help='whether use ntk in position encoding')
  group.add_argument('--ntk_scale_ratio',
                     default=2.0,
                     type=float,
                     help='ntk scale ratio')
