...
def run_test(
    ...
    # extra profiling of ranks
    pp_last_rank = mpu.get_pipeline_model_parallel_last_rank()
    pp_first_rank = mpu.get_pipeline_model_parallel_first_rank()
    tp_src_rank = mpu.get_tensor_model_parallel_src_rank()
    dp_src_rank = mpu.get_data_parallel_src_rank()
    is_pp_last_src = mpu.is_last_stage_and_scr_rank()
    is_pp_first_src = mpu.is_first_stage_and_scr_rank()
    ...
    info = f"="*20 + \
            ...
            f"> pp_group={pp_group}\n" + \
            f"> dp_group={dp_group}\n" + \
            f"> pp_last_rank={pp_last_rank}\n" + \
            f"> pp_first_rank={pp_first_rank}\n" + \
            f"> tp_src_rank={tp_src_rank}\n" + \
            f"> dp_src_rank={dp_src_rank}\n" + \
            f"> is_pp_first={is_pp_first}\n" + \
            f"> is_pp_first_src={is_pp_first_src}\n" + \
            f"> is_pp_last={is_pp_last}\n" + \
            f"> is_pp_last_src={is_pp_last_src}\n"
