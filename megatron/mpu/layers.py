...
class VocabParallelEmbedding(torch.nn.Module):
  ...
  def __init__(self,
    ...
    # args = get_args()
    self.use_bnb_optimizer = args.use_bnb_optimizer
    self.params_dtype = args.params_dtype
    self.embed_layernorm = args.embed_layernorm
    self.use_cpu_initialization = args.use_cpu_initialization
    ...
    if mpu.is_pipeline_first_stage() and (self.use_bnb_optimizer
                                          or self.embed_layernorm):
      self.norm = LayerNorm(embedding_dim,args=args)
  ...
  def forward(self, input_):
    # print('sfgdfg',input_.shape,input_.dtype,self.num_embeddings)
...
class ColumnParallelLinear(torch.nn.Module):
  ...
  def forward(self, input_):
    ...
    # print('input_parallel',input_parallel[:2,:2])
    # print('qkvweight',self.weight.shape,self.weight[:2,:2],self.weight.mean())
    # print('qkvbais',self.bias.shape,self.bias[:2],self.bias.mean())

    # Matrix multiply.

    bias = self.bias if not self.skip_bias_add else None
    output_parallel = F.linear(input_parallel, self.weight, bias)
    # print('output_parallel',output_parallel[:2,:2],output_parallel.mean())
    ...
    # print('output',output[:2,:2],output.mean())
...
class LoraColumnParallelLinear(ColumnParallelLinear):
    ...
    if self.use_lora and self.lora_rank > 0:
      self.lora_A = torch.nn.Parameter(
        self.weight.new_zeros(
          (self.lora_rank, input_size),
          device=torch.cuda.current_device()
          ),
        )
      # 注意这里的output是output_size_per_partition
      ...
      self.lora_dropout = torch.nn.Dropout(self.lora_dropout)
  ...
  def forward(self, input_):
    ...
    # 使用lora策略, 加上lora模块部分
    if self.use_lora and self.lora_rank > 0:
      # 这里lora_a需要保持每一个tp并行一致
      # print('before lora_a sum',self.lora_A.sum())
      torch.distributed.all_reduce(self.lora_A,
                                   op=torch.distributed.ReduceOp.SUM,
                                   group=mpu.get_tensor_model_parallel_group())
      self.lora_A.data  /= mpu.get_tensor_model_parallel_world_size()
      # 
      # print('lora_a sum',self.lora_A.sum())
      lora_A = self.lora_A
      output_parallel += (self.lora_dropout(input_) @ lora_A.T @ self.lora_B.T) * self.scaling
...
class RowParallelLinear(torch.nn.Module):
  ...
  def __init__(self,
    ...
    self.use_cpu_initialization = args.use_cpu_initialization
    self.params_dtype = args.params_dtype
    self.sync_tp_duplicated_parameters = args.sync_tp_duplicated_parameters
    from megatron import print_with_rank
    # print_with_rank(f'> input_size: {input_size}, output_size: {output_size}, input_size_per_partition:{self.input_size_per_partition}')
    # Parameters.
    # Note: torch.nn.functional.linear performs XA^T + b and as a result
    # we allocate the transpose.
    # Initialize weight.
    ...
    self.bias_tp_auto_sync = self.sync_tp_duplicated_parameters
