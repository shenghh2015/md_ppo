...
_LAST_TENSOR_MODEL_PARALLEL_GROUP = None
...
def initialize_model_parallel(tensor_model_parallel_size_=1,
  ...
  for i in range(num_tensor_model_parallel_groups):
    ...
    # if i == num_tensor_model_parallel_groups-1:
  ...
  for i in range(num_pipeline_model_parallel_groups):
    ...
    # print(f'rank: {rank}, embedding_ranks: {embedding_ranks},ranks: {list(ranks)},i: {i}')
    group = torch.distributed.new_group(embedding_ranks)
    if rank in embedding_ranks:
      _EMBEDDING_GROUP = group
      global _EMBEDDING_RANKS
      _EMBEDDING_RANKS = embedding_ranks
   
  from megatron import print_with_rank,get_local_ip
  print_with_rank(f'current rank ip: {get_local_ip()}\n')
...
def get_embedding_ranks():
  """Get the embedding group ranks the caller rank belongs to."""
  assert _EMBEDDING_RANKS is not None, \
      'embedding group is not initialized'
...
def is_last_stage_and_scr_rank():
  ...
  """
  return  is_pipeline_last_stage() and \
    dist.get_rank()==get_tensor_model_parallel_src_rank()

def is_first_stage_and_scr_rank():
  """判断当前rank是否属于第一个stage,并且在张量并行中排第一
  """
  return is_pipeline_first_stage() and \
    dist.get_rank()==get_tensor_model_parallel_src_rank()

def is_tensor_model_parallel_src_rank():
  return dist.get_rank()==get_tensor_model_parallel_src_rank()
