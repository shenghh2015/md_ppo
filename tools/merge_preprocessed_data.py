...
def get_args():
  ...
  group.add_argument(
      ...
      type=str,
      default='serial',
      choices=['serial', 'distributed'],
      help=
      'Whether to use a serial merge with a single process or a distributed parallel merge.'
  )
  ...
  if args.merge == 'serial' and args.torch_backend is not None:
    print_rank_0(
        "Ignoring setting for --torch-backend since using a serial merge")
...
def main():
  ...
  if args.merge == 'distributed':
    if args.distctx.numranks > len(args.datasets):
      print_rank_0(
          f"Using more ranks {args.distctx.numranks} than datasets {len(args.datasets)}"
      )
    ...
    first_dataset = indexed_dataset.make_dataset(args.datasets[0],
                                                 dataset_impl)
    dtype = first_dataset.dtype if isinstance(first_dataset,
                                              MMapIndexedDataset) else None
