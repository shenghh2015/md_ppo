...
def get_args():
  ...
  group.add_argument('--ref-model-name',
                     type=str,
                     help='refrence model name')
...
def ref_model_initialize(args):
  ...
  if MODEL is None:
    ...
    device  = _get_device()
    print(f'processing: {process_index}, initializing model on device: {device}')
    # tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForCausalLM.from_pretrained(model_name,torch_dtype=torch.float16).to(f'cuda:{device}')
    print(f'processing: {process_index}, initialized model on device: {device}')
    MODEL = model
...
def ref_model_call(args,batch_data):
  ...
  chosen_ids_list = []
  rejected_ids_list = []
  for prompt_ids,chosen_ids,rejected_ids in zip(prompt_batch,chosen_batch,rejected_batch):
  ...
  input_ids = pad_sequence(input_ids_list,batch_first=True,padding_value=Encoder.tokenizer.pad).to(device)
  # input_ids = input_ids[:,:args.ref_seq_length]
  ...
  with torch.no_grad():
    ...
    for i in range(bs):
      ...
      chosen_logit = chosen_logits[i,prompt_lens[i]-1:prompt_lens[i]-1 + max_chosen_rejected_len]
      rejected_logit = rejected_logits[i,prompt_lens[i]-1:prompt_lens[i]-1 + max_chosen_rejected_len]
       
      chosen_id = chosen_ids[i][prompt_lens[i]:prompt_lens[i] + max_chosen_rejected_len]
      rejected_id = rejected_ids[i][prompt_lens[i]:prompt_lens[i] + max_chosen_rejected_len]

      chosen_logprob = logprobs_of_labels(
        chosen_logit,
        chosen_id
      )

      rejected_logprob = logprobs_of_labels(
        rejected_logit,
        rejected_id
      )
...
def ref_model_stage(args,feats_iterator):
  ...
  pool = multiprocessing.Pool(
    worker_num,initializer=ref_model_initialize,initargs=(args,)
  )
  iters = pool.imap(partial(ref_model_call,args),batch_iterator(feats_iterator,args.ref_batch_size))
  for iter_ in iters:
...
def save_stage(args,res_iterator):
  ...
  save_path = os.path.join(args.output + f'{os.path.basename(args.input)}.feat.sample_num.{len(ret)}.pkl')
