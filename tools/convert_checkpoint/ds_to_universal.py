  ...
  for param_group_id in range(param_groups_cnt):
    ...
    for name, fragment_mapping in param_slice_mappings[param_group_id].items():
      ...
      for state_key in flat_state.keys():
        dump_param_fragment(dir, tp_index, dp_index, state_key,
...
def dump_param_fragment(dir, tp_index, dp_index, state_name, state_flat_tensor,
                        param_name, offset, numel):
...
def _strip_vocab_padding(ds_checkpoint, padded_vocab_tensor):
  ...
  padding_tensor = padded_vocab_tensor.narrow(
      0, checkpoint_info[ORIGINAL_VOCAB_SIZE],
...
def merge_tp_slices(ds_checkpoint, dir, slice_dir, tp_degree, name_and_shape):
  ...
  for state in ("fp32", "exp_avg", "exp_avg_sq"):
    ...
    if any(re.match(pattern, name) for pattern in WEIGHTS_TO_AVERAGE_PATTERNS):
      ...
      cat_dim = 1 if any(
          text in name for text in WEIGHTS_WITH_ROW_PARALLELISM_CONTAIN) else 0
    ...
    if "word_embeddings.weight" in name:
      ...
      ckpt_dict[
          'vocab_divisibility_padding_tensor'] = _get_vocab_divisibility_padding_tensor(
              ds_checkpoint, param)
...
def _extract_zero_shard_files(args, ds_checkpoint, slice_shapes, temp_dir):
  _3d_range_list = list(
      itertools.product(range(ds_checkpoint.pp_degree),
...
def _merge_tp_slice_files(args, ds_checkpoint, slice_shapes, temp_dir):
  work_chunks = list(
      _get_chunks(list(slice_shapes.items()), args.num_merge_workers))
  #pprint(work_chunks)
  zero_output_folder = os.path.join(args.output_folder, "zero")
  do_work = partial(merge_tp_slices, ds_checkpoint, zero_output_folder,
                    temp_dir, ds_checkpoint.tp_degree)
...
def main():
  ...
  ds_checkpoint = DeepSpeedCheckpoint(
      args.input_folder)  #, 1, 2) # args.target_tp, args.target_pp)
