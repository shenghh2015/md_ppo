...
from .constants import (BASE_OPTIMIZER_STATE, GROUP_PADDINGS,
                        OPTIMIZER_STATE_DICT, PARTITION_COUNT)

from .reshape_utils import (basic_folder_validation, get_zero_files,
                            merge_state)
...
class ZeROCheckpoint(object):
  ...
  def get_file_indices_for_rank(self, pp_index, tp_index, dp_index):
    assert dp_index < len(
        self._3d_file_map
    ), f'DP index {dp_index} >= DP degree {len(self._3d_file_map)}'
  ...
  def get_files_for_rank(self, pp_index, tp_index, dp_index):
    file_idx_list = self.get_file_indices_for_rank(pp_index, tp_index,
                                                   dp_index)
