...
from .reshape_utils import (basic_folder_validation, merge_state,
                            partition_data, get_files, get_files_with_prefix)

from .constants import (MODEL_FILE_PREFIX, LAYER_FILE_PREFIX, ZERO_FILE_PREFIX)
...
FIRST_LAYER_FILE_PREFIX = 'layer_01-model_0'
...
class DeepSpeedCheckpoint(object):
  def __init__(self, dir, tp_degree=None, pp_degree=None, dp_degree=None):
    ...
    self.first_layer_files = get_files_with_prefix(self.file_list,
                                                   FIRST_LAYER_FILE_PREFIX)
    self.mp_rank_files = get_files_with_prefix(self.file_list,
                                               MODEL_FILE_PREFIX)

    self.src_3d = self.get_model_3d_descriptor()
    self.layer_keys = self._get_layer_keys()
    self.layer_count = len(self.layer_keys)

    self.old_tp_degree = self.src_3d.tp_degree
    self.old_pp_degree = self.src_3d.pp_degree
    # self.old_dp_degree = self.src_3d.dp_degree

    self.tp_degree = self.src_3d.tp_degree if tp_degree is None else tp_degree
    self.pp_degree = self.src_3d.pp_degree if pp_degree is None else pp_degree
    # self.dp_degree = self.src_3d.dp_degree if dp_degree is None else dp_degree
    ...
    # self.original_world_size = self.old_tp_degree * self.old_pp_degree * self.old_dp_degree
    # assert self.original_world_size == self.gpus_num
    # self.world_size = self.tp_degree * self.pp_degree * self.dp_degree

    self.old_2d_map = meg_2d_parallel_map(self.old_pp_degree,
                                          self.old_tp_degree)
    self.old_2d_map.simple_init()
    self.new_2d_map = reshape_meg_2d_parallel(old_pp_degree=self.old_pp_degree,
                                              old_tp_degree=self.old_tp_degree,
  ...
  def _sanity_check(self):
    ...
    # assert self.gpus_num % (self.pp_degree * self.tp_degree) == 0
  ...
  def _validate_folder(self, dir):
    ...
    for file_prefix in [
        MODEL_FILE_PREFIX, LAYER_FILE_PREFIX, f'{LAYER_FILE_PREFIX}01'
    ]:
  ...
  def get_model_3d_descriptor(self):
    file_list = self.file_list
    # mp_rank_file_list = self.mp_rank_files
    num_pp0_files = len(
        get_files_with_prefix(file_list, f'{LAYER_FILE_PREFIX}01'))
    if num_pp0_files > 0:
      tp_degree = num_pp0_files
      pp_degree = len(get_files_with_prefix(file_list,
                                            MODEL_FILE_PREFIX)) // tp_degree
      # dp_degree = max(1, self.gpus_num // (pp_degree * tp_degree))
    else:
      tp_degree = len(get_files_with_prefix(file_list, MODEL_FILE_PREFIX))
      # dp_degree = max(1, self.gpus_num // tp_degree)
      pp_degree = 0

    # return model_3d_desc(pp_degree, tp_degree, dp_degree)
    return model_3d_desc(
        pp_degree,
        tp_degree,
    )
