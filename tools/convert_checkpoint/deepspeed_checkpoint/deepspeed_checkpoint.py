...
class DeepSpeedCheckpoint(object):
  def __init__(self, dir, tp_degree=None, pp_degree=None, dp_degree=None):
    ...
    self.old_2d_map = meg_2d_parallel_map(
        self.zero_checkpoint.get_src_pp_degree(),
    ...
    if self.is_change_pp_degree() or self.is_change_tp_degree(
      ...
      self.zero_checkpoint.reshape(
          model_3d_desc(self.pp_degree, self.tp_degree, self.dp_degree))
  ...
  def get_zero_checkpoint_state(self, pp_index, tp_index, dp_index) -> dict:
    return self.zero_checkpoint.get_state_for_rank(
        pp_index=pp_index,
        tp_index=tp_index,
        dp_index=dp_index,
  ...
  def _sanity_check(self):
    ...
    assert self.zero_checkpoint.num_files % (self.pp_degree *
                                             self.tp_degree) == 0
