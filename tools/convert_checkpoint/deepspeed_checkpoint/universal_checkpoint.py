...
import torch
import types
from .constants import (FP32_WEIGHT_KEY, PARAM,
                        VOCAB_DIVISIBILITY_PADDING_TENSOR, CAT_DIM)
...
def load_hp_checkpoint_state(self, folder, tp_rank, tp_world_size):
  ...
  checkpoint_files = {
      key: os.path.join(folder, f"{key}.pt")
      for key in hp_keys
  }
  ...
  for key in hp_keys:
    ...
    vocab_divisibility_padding_tensor = ckpt_dict.get(
        VOCAB_DIVISIBILITY_PADDING_TENSOR, None)
    ...
    tp_hp_fragment = tp_hp_slice.narrow(0, lp_frag_address.start,
