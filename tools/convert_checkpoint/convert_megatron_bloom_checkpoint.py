...
# PYTHONPATH=outputs/tmp/Megatron-LM python src/transformers/models/megatron_gpt2/convert_megatron_gpt2_checkpoint.py ...
#
...
from transformers import AutoTokenizer, GPT2Config, BloomConfig
...
def fix_query_key_value_ordering(param, checkpoint_version, num_splits,
...
def convert_megatron_checkpoint(args, input_state_dict, config):
  ...
  output_state_dict[
      "transformer.word_embeddings_layernorm.weight"] = embeddings[
          "word_embeddings"]['norm.weight']
  output_state_dict["transformer.word_embeddings_layernorm.bias"] = embeddings[
  ...
  # The transformer.
  transformer = lm["transformer"] if "transformer" in lm.keys(
  ) else lm["encoder"]

  # The regex to extract layer names.
  layer_re = re.compile("layers\.(\d+)\.([a-z0-9_.]+)\.([a-z_A-z]+)")

  # layers.69.self_attention.query_key_value.weight
  # layers.69.self_attention.query_key_value.lora_A
  ...
  for key, val in transformer.items():
    ...
    print('op_name and weight_or_bias', op_name, weight_or_bias)
    ...
    # Transpose the QKV matrix.
      ...
      # print(val, val.shape)
      out_val = fix_query_key_value_ordering(val, checkpoint_version, 3, heads,
      ...
      output_state_dict[layer_name +
                        ".self_attention.query_key_value.weight"] = val
    ...
    # Transpose the bias.
      ...
      output_state_dict[layer_name +
                        ".self_attention.query_key_value.bias"] = val
  ...
  # The final layernorm.
  output_state_dict["transformer.ln_f.weight"] = transformer[
      "final_layernorm.weight"]
  output_state_dict["transformer.ln_f.bias"] = transformer[
...
def main():
  ...
  if args.config_file == "":
    ...
    config = BloomConfig.from_json_file(args.config_file)
