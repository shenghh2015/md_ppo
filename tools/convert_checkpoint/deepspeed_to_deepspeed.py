...
def _create_transformer_layer_checkpoint(ds_checkpoint, base_folder, tp_index,
  ...
  for sd, layer_id in zip(sd_list, layer_id_list):
    ckpt_path = get_layer_ckpt_name_for_rank(base_folder=base_folder,
                                             layer_id=layer_id,
                                             tp_rank=tp_index)
...
def _strip_vocab_padding(ds_checkpoint, padded_vocab_tensor):
  ...
  unpadded_vocab_tensor = torch.narrow(padded_vocab_tensor, 0, 0,
                                       target_args.padded_vocab_size)
...
def _create_embedding_layer_checkpoint(ds_checkpoint, base_folder, tp_index):
  ...
  if ds_checkpoint.is_change_tp_degree():
    sd[WORD_EMBEDDINGS_KEY] = _strip_vocab_padding(ds_checkpoint,
                                                   sd[WORD_EMBEDDINGS_KEY])
...
def _create_zero_checkpoint(ds_checkpoint, base_folder, dp_index, pp_index,
                            tp_index):
...
def main():
  ...
  ds_checkpoint = DeepSpeedCheckpoint(args.input_folder, args.target_tp,
                                      args.target_pp, args.target_dp)
  iteration = ds_checkpoint.get_iteration()
  latest_tag = f'global_step{iteration}'
  _create_latest_file(args.output_folder, 'latest_checkpointed_iteration.txt',
                      iteration)
