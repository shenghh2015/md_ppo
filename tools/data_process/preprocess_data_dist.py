...
def get_args():
  ...
  group.add_argument('--input', type=str, required=True, help='Dataset name')
  ...
  group.add_argument('--torch-backend',
                     type=str,
                     default='gloo',
                     choices=['gloo', 'mpi'],
  ...
  group.add_argument(
      ...
      default=None,
      help=
      'Local rank of calling process on its node (from torch.distributed.launch).'
  )
  group.add_argument(
      '--merge',
      type=str,
      ...
      help=
      ('Method to merge intermediate per-rank files into the final data files.  '
  ...
  group.add_argument(
      ...
      default=None,
      help=
      ('Path to local storage on compute nodes to write per-rank files before merging, like /dev/shm.  '
  ...
  if args.scratch is not None and args.merge != 'parallel':
    raise ValueError(
        "The --scratch option is only valid with --merge=parallel")
...
def load_dset(args):
  ...
  if args.rank == 0:
    ...
    try:
      ...
      msgerr(
          f"If the dataset is large, it may be more efficient to download with a single process:"
      )
      msgerr(f"    from datasets import load_dataset")
      msgerr(f"    dset = load_dataset('{dsetname}')")
      msgerr(
          f"Alternatively, one can force this script to download by setting $HF_DATASETS_OFFLINE=0",
          flush=True)
      err = e
...
def get_proc_counts(num, num_ranks):
  ...
  return [
      num_per_rank + 1 if rank < remainder else num_per_rank
...
def rank_files_write(args, dset, idx, encoder):
  ...
  try:
    ...
    for key in args.columns:
      ...
      best_dtype = best_fitting_dtype(
          args.padded_vocab_size) if args.dataset_impl == "mmap" else None
    ...
    for i in idx:
      ...
      if args.rank == 0 and args.log_interval > 0 and time.time(
      ) > progress_next:
        current = time.time()
        ...
        mbs = dset_stats[
            2] * args.numranks / elapsed / 1024 / 1024 if elapsed > 0.0 else 0.0
        secs_left = int((num_samples - docs) /
                        docrate if docrate > 0.0 else 0.0)
        msg(
            f"Processed (estimated) {docs} of {num_samples} docs ({percent:0.2f}%) in {int(elapsed)} secs, "
  ...
  if args.rank == 0:
    ...
    secs_read_per_sample = times[0] / dset_stats[0] if dset_stats[
        0] > 0 else 0.0
    secs_encode_per_sample = times[1] / dset_stats[0] if dset_stats[
        0] > 0 else 0.0
    secs_write_per_sample = times[2] / dset_stats[0] if dset_stats[
        0] > 0 else 0.0
    msg("Process stats:")
...
def main():
  ...
  try:
    ...
    if args.rank == 0:
      msgerr(
          f"At least one process failed to write its file, skipping merge and cleaning up",
          flush=True)
