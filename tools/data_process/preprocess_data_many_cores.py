...
def process_samples(simple_queue, process_id, args, level, writer: Connection):
  ...
  for key in args.json_keys:
    output_filename = get_output_filename(args.output_prefix, key, level,
                                          process_id)
...
def log(readers, log_interval):
  ...
  bytes_queue.extend([(proc_start, total_bytes_processed)] *
                     bytes_queue_max_length)
  ...
  while len(readers) != 0:
    for r in multiprocessing.connection.wait(readers):
      ...
      if data[0] is None:
        ...
        readers.remove(r)
        print(
            f"Process {process_index} finished working. Remaining workers: {len(readers)}",
            flush=True)
        continue
      ...
      if (doc_processed - logged_docs) >= log_interval:
        ...
        mbs = (total_bytes_processed -
               old_bytes) / (current - old_start_time) / 1024 / 1024
...
def main():
  ...
  readers, writers = list(
      zip(*[
          multiprocessing.Pipe(duplex=False) for _ in range(args.workers - 1)
      ]))
  process_ids = list(range(len(writers)))
  processes = [
      multiprocessing.Process(target=process_samples,
                              args=(simple_queue, process_id, args, level,
                                    writer))
      for process_id, writer in zip(process_ids, writers)
  ]
  log_thread = threading.Thread(target=log,
                                args=(list(readers), args.log_interval))
  fill_thread = multiprocessing.Process(target=fill_simple_queue,
                                        args=(args.input, simple_queue,
                                              chunk_size))
  ...
  for process in processes:
    ...
    process.close()
  log_thread.join(
  )  #TODO: figure out why there seems to be a possible dead lock situation.
